...
def validate_model(generator, val_loader, device):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(val_loader):
            ...
            # 배치별 시각화 (예: 100번째 배치마다)
            if batch_idx % 100 == 0 or batch_idx == 0:  # 첫 배치와 100번째마다 저장
                vis_inputs = np.abs(inputs.cpu().numpy())  # 복소수 절대값
                ...
                visualize_images(
                    ...
                    vis_targets,
                    epoch=0,
                    save_dir='visualizations/wgan_valid',
                    batch_idx=batch_idx,
                    dataset_name=f'wgan_validation_batch_{batch_idx}'
                )
            if batch_idx % 100 == 0:
...
def find_best_checkpoint(checkpoint_dir='checkpoints'):
    ...
    for filename in os.listdir(checkpoint_dir):
        if filename.startswith('wgan_generator_epoch_') and filename.endswith('.pth'):
            ...
            with open('wgan_validation_metrics.txt', 'r') as f:
                for line in f:
                    if f'wgan_generator_epoch_{epoch}.pth' in line:
                        psnr = float(line.split('PSNR: ')[1].split(',')[0])
                        if psnr > best_psnr:
                            best_psnr = psnr
                            best_epoch = epoch
    return f'checkpoints/wgan_generator_epoch_{best_epoch}.pth' if best_epoch > 0 else None
...
def main():
    ...
    # 최고 에포크 체크포인트 자동 선택 (wgan_validation_metrics.txt 기반)
    checkpoint_dir = '/home/compu/john/MAI-LAB/checkpoints'
    model_path = find_best_checkpoint(checkpoint_dir)
    if not model_path:
        print("No valid checkpoint found or wgan_validation_metrics.txt not available. Using default epoch 9.")
        model_path = 'checkpoints/wgan_generator_epoch_2.pth'
    ...
    # 결과 출력 및 저장
    print(f'WGAN Validation Results - Model: {os.path.basename(model_path)}, '
          f'Avg PSNR: {avg_psnr:.4f}, Avg SSIM: {avg_ssim:.4f}')
    with open('wgan_validation_metrics.txt', 'a') as f:
        f.write(f'{os.path.basename(model_path)} - PSNR: {avg_psnr:.4f}, SSIM: {avg_ssim:.4f}\n')

    # 튜닝 피드백 로그
    with open('wgan_validation_feedback.txt', 'a') as f:
        f.write(f'Model: {os.path.basename(model_path)}, PSNR: {avg_psnr:.4f}, SSIM: {avg_ssim:.4f}\n')
        f.write(f'Suggestion: Adjust lambda_gp or n_critic if PSNR < 25, consider increasing epochs if SSIM < 0.9\n')
